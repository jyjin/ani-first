...
import { PERSONAL_GOOD_LIST } from '../../redux/action/actionTypes';
...
class Hot extends Component {
    ...
    onScroll = () => {
        ...
        // var el = document.getElementById('scrollPersonWrap')
        var el = document.getElementsByClassName('ant-tabs-tabpane ant-tabs-tabpane-active')[0]
        el.onscroll = () => {
    ...
    getGoodList(down) {
        ...
        api.getGoodList({ page: this.state.page, userId: this.props.user._id }).then(json => {
            if (json.res > 0) {
                ...
                Promise.all(pros).then(result => {
                    this.props.dispatch(PERSONAL_GOOD_LIST, { personalGoodList: result[result.length - 1].data, prevPersonalGoodList: down ? [] : this.props.prevPersonalGoodList || [] })
                    if (down) {
                        this.setState({
                            loadingDown: false
                        })
    ...
    render() {
        ...
        console.log('personal == ', this.props.personalGoodList)
        return (
            <div id="scrollPersonWrap" style={Object.assign({}, style.wrap)}>
                {
                ...
                {
                    this.props.personalGoodList && this.props.personalGoodList.length && this.props.personalGoodList.map((item, itemIndex) => {
                ...
                {
                    this.state.loading ? <div style={{ display: 'flex', width: '100%', justifyContent: 'center', paddingTop: '20px' }}>
